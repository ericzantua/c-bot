...
- **Vercel env vars** (same keys minus scraper/CDP): `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `VOYAGE_API_KEY`, `ELEVENLABS_API_KEY(S)`, `TTS_PROVIDER`, plus `AUTH_USERS` (login accounts, `user:pass,user2:pass2`) + `AUTH_SECRET` (random cookie-signing key) — redeploy after changing.
