...
├── middleware.js          Vercel Edge Middleware — cookie-login gate over the WHOLE site (PWA + /api/*); /login page + signed session cookie; creds from AUTH_USERS, signed with AUTH_SECRET
...
- **App is PRIVATE — cookie-based login (DONE, verified live)**: `middleware.js` (Vercel Edge Middleware, `matcher: /((?!_vercel/).*)`) gates every request — static PWA and `/api/*`. Unauthenticated navigations get **303 → `/login`** (a self-contained branded HTML page served by the middleware); `/api/*` gets **401 JSON**. `POST /login` verifies user+pass and sets a **signed, HttpOnly, Secure, SameSite=Lax** cookie `cbot_session` (HMAC-SHA256 via Web Crypto, 30-day expiry); the middleware verifies it on every request. `/logout` clears it (Log-out link in the App top nav). Config = **`AUTH_USERS`** (comma-separated `user:pass` pairs; avoid `,`/`:` in passwords) + **`AUTH_SECRET`** (random HMAC key); **fail-closed** 503 if either is unset. Constant-time password compare. Cookies auto-sent with same-origin `/api/*` fetches (incl. service-worker requests) → no double-prompt, works in installed PWAs (unlike the earlier Basic-Auth approach, which double-prompted). Set/rotate creds in the Vercel dashboard (never in the repo), then **redeploy** (env changes need a redeploy). `parseUsers()` still falls back to the legacy `BASIC_AUTH_USERS` var if present.
- **Roles: admin vs plain user (DONE, verified)**: admins listed in **`AUTH_ADMINS`** env var (comma-separated usernames). The middleware exposes **`GET /api/me` → `{user, admin}`** (the SPA reads its identity from here since the cookie is HttpOnly) and enforces admin-only APIs server-side: non-admins get **403** on `/api/(settings|products|index|knowledge)*`; everyone keeps `/api/chat`, `/api/tts`, `/api/health`, `/api/me`. Frontend mirrors this — admin sees Products/Settings tabs, non-admin sees only Z-Bot + About; App also bounces a non-admin off any admin-only view. Current accounts: `eric` (admin) + `w552maj` (plain user). About page = credits/app info (Eric Zantua, Costco WH-552, 552 Major Sales).
...
- **Vercel env vars** (same keys minus scraper/CDP): `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `VOYAGE_API_KEY`, `ELEVENLABS_API_KEY(S)`, `TTS_PROVIDER`, plus `AUTH_USERS` (login accounts, `user:pass,user2:pass2`), `AUTH_SECRET` (random cookie-signing key), `AUTH_ADMINS` (comma-separated admin usernames, e.g. `eric`) — redeploy after changing. **All set in production + chat verified live (2026-07-29).** ⚠️ **Gotcha**: Vercel does NOT bundle `backend/.env` into the serverless function — every key MUST be set as a Vercel env var (dashboard or `printf VAL | vercel env add NAME production`), else prod runs with empty keys (symptom: `VOYAGE_API_KEY is not set` on first chat, since `config.py` has no fallbacks). `load_dotenv()` uses `override=False`, so Vercel env vars win over any `.env`.
