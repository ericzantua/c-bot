...
├── middleware.js          Vercel Edge Middleware — HTTP Basic Auth gate over the WHOLE site (PWA + /api/*); creds from BASIC_AUTH_USERS env var
...
- **App is PRIVATE — HTTP Basic Auth (DONE)**: `middleware.js` (Vercel Edge Middleware, `matcher: /((?!_vercel/).*)`) gates every request — static PWA and `/api/*` — before it's served. Credentials = `BASIC_AUTH_USERS` env var, comma-separated `user:pass` pairs (supports several accounts); avoid `,`/`:` in passwords. **Fail-closed**: if the var is unset the whole site returns 503. Constant-time password compare. Set/rotate creds in the Vercel dashboard (never in the repo), then **redeploy** (env changes need a redeploy to take effect). Browser sends the Authorization header automatically to same-origin `/api/*` after login, so no frontend changes. ⚠️ Caveat: installed iOS PWAs can be flaky with Basic Auth in standalone mode; fallback = cookie-based login page if it misbehaves. Verified live: no/wrong creds → 401, correct → 200.
...
- **Vercel env vars** (same keys minus scraper/CDP): `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `VOYAGE_API_KEY`, `ELEVENLABS_API_KEY(S)`, `TTS_PROVIDER`, plus `BASIC_AUTH_USERS` (auth gate; `user:pass,user2:pass2` — redeploy after changing).
